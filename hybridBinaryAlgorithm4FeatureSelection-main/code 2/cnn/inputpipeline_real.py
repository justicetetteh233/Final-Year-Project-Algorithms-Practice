    ...
    def get_train_input(self):
        ...
        for file in os.listdir(self.input_dataset):
            if self.num_channels > 1:
                filelabel=file.split('.')[0]
                filelabel=filelabel.split('_')[0]
            else:
                filelabel=(file.split('_')[-1]).split('.')[0]
            
            if filelabel == 'N':
                sizeOfNinDatasets=sizeOfNinDatasets+1 #Size of 'N' in mammo/histo 
                
        #No. of data to augment: Remove the 'N' and multiply the remaining size with nDuplication
        effectof_sizeOfNinDatasets=(nFiles - sizeOfNinDatasets) * nDuplication
        #Add the actual datasize to number to be augmented
        nFiles=nFiles+effectof_sizeOfNinDatasets
        
        if self.num_channels == 3:
            img_data_array, dim=self.init_imgs(nFiles)            
            class_name = np.empty((nFiles, ))            
            num_imgs_per_label={'F': 0, 'DC': 0, 'LC': 0, 'MC': 0, 'IS': 0,
                                'A': 0, 'PT': 0, 'PC': 0, 'TA': 0, 'B': 0, 'IV': 0, 'N': 0}
        else:
            nDuplication=2
            img_data_array, dim=self.init_imgs(nFiles)            
            class_name = np.empty((nFiles, ))
        ...
        print(nFiles, effectof_sizeOfNinDatasets)
        n=0
        prev_label=''
        ...
        for file in os.listdir(self.input_dataset):
            if file.endswith(('jpg', 'png')):
                ...
                #Data augmentation
                M = np.float32([[1,  0, 0], [0, -1, zrows], [0,  0, 1]])
                reflected_img = cv2.warpPerspective(image, M, (int(zcols),int(zrows)))
                tmpfile1=tmpf+'refelected.png'
                image_path1= os.path.join(self.input_dataset, tmpfile1)
                #cv2.imwrite(image_path1, reflected_img)
                
                #flip horizontally
                M = np.float32([[1,  0, 0], [0, -1, zrows], [0,  0, 1]])
                img_rotation = cv2.warpAffine(image, cv2.getRotationMatrix2D((zcols/2, zrows/2), 30, 0.6), (zcols, zrows))
                tmpfile2=tmpf+'rotated.png'
                image_path2= os.path.join(self.input_dataset, tmpfile2)
                #cv2.imwrite(image_path2, img_rotation)
                
                #flip vertically
                M = np.float32([[-1, 0, zcols], [0, 1, 0], [0, 0, 1]])
                reflected_img2 = cv2.warpPerspective(image, M, (int(zcols),int(zrows)))
                
                img_rotation2 = cv2.warpAffine(image, cv2.getRotationMatrix2D((zcols/2, zrows/2), 60, 0.4), (zcols, zrows))
                
                img_rotation3 = cv2.warpAffine(image, cv2.getRotationMatrix2D((zcols/2, zrows/2), 90, 0.4), (zcols, zrows))
                ...
                #subsequent iteration of for-loop where label has changed
                if (prev_label != label and prev_label !='') and (prev_label != label and n > 0):
                    #num_imgs_per_label[prev_label]=label_count
                    num_imgs_per_label[label]=num_imgs_per_label[label] +1
                    #print('+++++++++++++++++++++++++++++++++++++++++++++++++')
                    #print(num_imgs_per_label)
                    #print('+++++++++++++++++++++++++++++++++++++++++++++++++')
                    prev_label=label
                    #label_count=1
                else:
                ...
                '''
                if self.num_channels > 1:
                    label = self.classes[tmpf] #, self.num_classes)
                    
                    img_data_array[n, :, :, :] = image
                    class_name[n] = label
                    n=n+1
                    
                    #To take note of keeping count of number of 'N' labels in the dataset
                    if tmpf =='N':
                        num_imgs_per_label[tmpf]=num_imgs_per_label[tmpf] + 1
                    
                    if  tmpf != 'N':                                            
                        reflected_img=np.resize(reflected_img, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = reflected_img
                        class_name[n] = label
                        n=n+1
                        
                        img_rotation=np.resize(img_rotation, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = img_rotation
                        class_name[n] = label
                        n=n+1
                        
                        reflected_img2=np.resize(reflected_img2, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = reflected_img2
                        class_name[n] = label
                        n=n+1
                        
                        img_rotation2=np.resize(img_rotation2, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = img_rotation2
                        class_name[n] = label
                        n=n+1
                        
                        img_rotation3=np.resize(img_rotation3, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = img_rotation3
                        class_name[n] = label
                        n=n+1
                        
                        #To take note of keeping count of number of other labels in the dataset
                        num_imgs_per_label[tmpf]=num_imgs_per_label[tmpf] + nDuplication
                    ...
                    elif n%4==0 and tmpf != 'N':
                        img_rotation=np.resize(img_rotation, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = img_rotation
                        class_name[n] = label
                        n=n+1
                    
                    elif n%5==0 and tmpf != 'N':
                        reflected_img2=np.resize(reflected_img2, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = reflected_img2
                        class_name[n] = label
                        n=n+1
                    
                    elif n%7==0 and tmpf != 'N':
                        img_rotation2=np.resize(img_rotation2, (zrows, zcols, self.num_channels))
                        img_data_array[n, :, :, :] = img_rotation2
                        class_name[n] = label
                        n=n+1
        ...
        '''
        self.train_split = int(self.train_split * img_data_array.shape[0])
        self.test_split = int(self.test_split * img_data_array.shape[0])
        self.eval_split = int(self.eval_split * img_data_array.shape[0])
