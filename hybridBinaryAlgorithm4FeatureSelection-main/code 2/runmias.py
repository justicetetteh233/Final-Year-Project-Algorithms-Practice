...
if __name__ == "__main__": 
    ...
    histocnn_paras={
            ...
            'fromCheckpoint':False, 'train_model': True,
    ...
    mamocnn_paras={
            ...
            'checkpoint_epoch':40, 'fromCheckpoint':True, 'train_model': True,
    ...
    if mamocnn_paras['train_model']:
        ...
        #mcnn.extract_vector_features(mcnn.model, xeval, yeval, x_test, y_test, 'pool_4')
    ...
    #lb, ub, x, y, tx, ty=mcnn.optimize_vector_features(xeval, yeval, x_test, y_test)
    
    '''
    y_pred=predict_prob(x)
    #collapse classes of histo into the 3-class model of the siemese CNN
    mammo_probmap_3labels, categoricaltrue=fusion_label_remap(y, y_pred, mammo_mias_classes.keys(), mammo_mias_classes.values(), mammo_named_classes, 'mammo') 
    item={'labels':mammo_probmap_3labels, 'categoricaltrue':categoricaltrue}
    save_results_to_csv(item, 'mammo_probmap_3labels', metrics_result_dir)
    '''
    #Send features to binary optimizer for optimization process
    ...
    results=make_prediction(new_x_train, y_train)
    #pr._save_classifiers_results__(algorithm, results, solution, best_position, best_fitness)
    #Predict using the model on the newly optimized features
    ...
    pop=generate_fusion_population(mammo_probmap_3labels, histo_probmap_3labels)
    probsmap_optimizer(pop, model_rates, pr)
